...
extern void set_trap_gate(int n, IntHandler *inthand_addr);
...
void init_proctab()
{
  ...
  for(i = 0;i < NPROC;i++){
    ...
    proctab[i].p_savedregs[SAVED_EFLAGS] = EFLAGS_IF;
    proctab[i].p_status=RUN;

    proctab[i].p_quantum = TEN_MIL_SEC;
    proctab[i].p_ticks = 0;
